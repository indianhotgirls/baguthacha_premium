...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/+a3QI1YnUItszOWJh'),
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not await db.has_premium_access(message.from_user.id):
        await message.reply_text("You are not premium member. Kindly contact us @DesileaksContactusBot to buy the membership")
        return 
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        text = get_original_string(data)
        pre, file_id = ((base64.urlsafe_b64decode(text + "=" * (-len(text) % 4))).decode("ascii")).split("_", 1)
        if not await check_verification(client, message.from_user.id) and VERIFY_MODE == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{username}?start="))
            ],[
        ...
        try:
            ...
            title = '@indian_sluts_leaks  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
